#!/usr/bin/env node
// Snapshot a Notion "brain" page into docs/brain-snapshot.md.
//
// Project-agnostic version (brain kit): set the page id via NOTION_BRAIN_PAGE_ID.
// CLAUDE.md is the canonical working brain; this produces a durable,
// git-versioned backup/mirror of the Notion brief so it survives Notion edits
// or retention limits.
//
// Fails loudly — never a silent success:
//   * missing NOTION_TOKEN or NOTION_BRAIN_PAGE_ID  -> exit 1
//   * Notion 401/403/404 (bad token / no access / page not shared) -> exit 1
//   * empty/partial page content -> exit 1 (won't overwrite a good backup)
//
// Usage:
//   NOTION_TOKEN=ntn_xxx NOTION_BRAIN_PAGE_ID=<page-id> node scripts/snapshot-notion-brain.mjs

import { writeFile, mkdir } from 'node:fs/promises';
import { dirname } from 'node:path';

const TOKEN = process.env.NOTION_TOKEN;
const PAGE_ID = process.env.NOTION_BRAIN_PAGE_ID;
const OUT = process.env.BRAIN_SNAPSHOT_PATH || 'docs/brain-snapshot.md';
...
if (!PAGE_ID) {
  console.error(
    '::error::NOTION_BRAIN_PAGE_ID is not set. Add it under repo Settings → ' +
      'Secrets and variables → Actions → Variables (the Notion brain page id).',
  );
  process.exit(1);
}
